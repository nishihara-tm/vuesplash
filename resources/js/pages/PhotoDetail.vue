<template>
  <div v-if="photo" class="photo-detail" :class="{ 'photo-detail--column': fullWidth }">
    <figure class="photo-detail__pane photo-detail__image" @click="fullWidth = ! fullWidth">
      ...
      <figcaption>Posted by {{ (photo.owner) ? photo.owner.name : '' }}</figcaption>
    </figure>
    <div class="photo-detail__pane">
      ...
      <ul v-if="photo.comments && photo.comments.length > 0" class="photo-detail__comments">
        <li
...
export default {
  ...
  methods: {
    ...
    async addComment () {
      ...
      const setData = (this.photo.comments && this.photo.comments.length > 0) ? [response.data, ...this.photo.comments] : [response.data]
